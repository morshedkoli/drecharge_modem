	...
	// recharge message execute end
  	...
  	if (!empty($slid) && !empty($msg)) {
  		...
  		if ($querycount && $querycount->num_rows > 0) {
			...
			$requestStatusResolved = $isRequestFailure ? 'Failed' : 'Success';
			...
			if ($isRequestFailure) {
				...
				$requestUpdateSql = "UPDATE reload_sent_number SET status = 'Failed', remark='Failed from android', trxid='" . $conn->real_escape_string($requestTrxidResolved) . "', topup_number='" . $conn->real_escape_string($simno) . "', sim_balance='" . $conn->real_escape_string($simam) . "' WHERE id = '$reload_id'";
			} else {
				$requestShouldCreditCommission = count($parents_commission) > 0;
				$requestUpdateSql = "UPDATE reload_sent_number SET status = 'Success', statusCom = '2', remark='Completed from android', topup_number='" . $conn->real_escape_string($simno) . "', sim_balance='" . $conn->real_escape_string($simam) . "'";
				if (!empty($requestTrxidResolved)) {
					$requestUpdateSql .= ", trxid='" . $conn->real_escape_string($requestTrxidResolved) . "'";
				}
				$requestUpdateSql .= " WHERE id = '$reload_id'";
			}
